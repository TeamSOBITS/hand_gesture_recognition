...
from sobits_interfaces.msg import StringArray
...
class HandSignRecognition(Node):
    def __init__(self):
        ...
        self.declare_parameter("keypoint_classifier_label","/home/sobits/colcon_ws/src/hand_gesture_recognition/hand_gesture_recognition/model/keypoint_classifier/keypoint_classifier_label.csv")
        self.declare_parameter("keypoint_classifier_model","/home/sobits/colcon_ws/src/hand_gesture_recognition/hand_gesture_recognition/model/keypoint_classifier/keypoint_classifier.tflite")
        self.keypoint_classifier_label = self.get_parameter("keypoint_classifier_label").get_parameter_value().string_value
        self.keypoint_classifier_model = self.get_parameter("keypoint_classifier_model").get_parameter_value().string_value
        ...
        self.pub_gesture = self.create_publisher(StringArray,"gesture_name",1)
    ...
    def image_cb(self,msg):
        ...
        gesture_msg = StringArray()
        gesture_msg.data=gestures
        self.pub_gesture.publish(gesture_msg)

        if hand_lms is not None:
            keypoint_array_msg = KeyPointArray()
            keypoint_array_msg.header = msg.header
            for hand_idx, (hand_lm, hand_pos, gesture, conf) in enumerate(zip(hand_lms, hand_poss, gestures, confs)):
